$(function () {
    ...
    $('.btnUpload').on('click', function () {
        ...
        $('.btnUpload', parent).html(loadingText);
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('.btnUpload', parent).html($('.btnUpload', parent).data('original-text'));
            },
            error: function (e) {
    ...
    function registerAutoNumeric() {
        donGiaBunInstall = new AutoNumeric('.dongiabun', { decimalPlaces: 0 });
        $('.dongiabun').on('keyup', function () {
            submitDonGiaBun();
        });
    ...
    function submitDonGiaBun() {
        $.ajax({
            type: "post",
            url: $('#hidSubmitDonGiaBun').val(),
            data: {
                thang: $('.hidThang').val(),
                price: $('.dongiabun').val()
            },
    ...
    function calculatorData(code) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.result === true) {
                    $('.luongtheodoanhthudoanhso-' + code).html(accounting.formatNumber(data.entity.luongTheoDoanhThuDoanhSo/1000));
                    $('.tongsochuyen-' + code).html(accounting.formatNumber(data.entity.tongSoChuyen));
                    $('.tienchuyen-' + code).html(accounting.formatNumber(data.entity.tienChuyen / 1000));
                    $('.congtacxa-' + code).html(accounting.formatNumber(data.entity.congTacXa / 1000));
                    $('.thanhtienbun-' + code).html(accounting.formatNumber(data.entity.thanhTienBun / 1000));
                }
                else {
                    console.log("Missing load data....");
                }
            }
        });
    }
